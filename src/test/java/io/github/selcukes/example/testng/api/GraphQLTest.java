...
public class GraphQLTest {
	...
	public void verifyCeoTest() {
		...
		page = Pages.apiPage();
		JsonNode body = page.request("https://api.spacex.land/graphql/").post(query).bodyJson().at("/data/company/ceo");
	...
	public void verifyMissionName() {
		...
		page = Pages.apiPage();
		JsonNode body = page.request("https://api.spacex.land/graphql/").post(query).bodyJson().at("/data/launches")
	...
	public void verifyUserData() {
		...
		User myUser = User.builder().id(UUID.randomUUID()).name("Selcukes").rocket("One Stop Automation").build();

		query.setVariables(myUser);

		page = Pages.apiPage();
		JsonNode body = page.request("https://api.spacex.land/graphql/").post(query).bodyJson()
