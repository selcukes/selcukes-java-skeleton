...
public class GraphQLTest {
	...
	public void verifyMissionName() {
		...
		page = Pages.apiPage();
		JsonNode body = page.request(API_URL).post(query).bodyJson().at("/data/launches").get(0).get("mission_name");
	...
	public void verifyUserData() {
		...
		page = Pages.apiPage();
		JsonNode body = page.request(API_URL).post(query).bodyJson().at("/data/insert_users/returning").get(0);
